...
std::cout << "starting test Update priorities!" << std::endl;
   ...
   //Sorted PQ
    BinaryPQ<const int*, IntPtrComp> binary1;
    vector<int> data1{
        6,
        3, 
        2,
    ...
    assert(*binary1.top() == 6);
    auto& datum1 = data1[2];
    datum1 = 7;
    binary1.updatePriorities();
    assert(*binary1.top() == 7);
    
    std::cout << "End of Update Priorities\n";
}
...
void testPairing() {
    ...
    std::cout << "testing updateElt" << std::endl;
        ...
        PairingPQ<int> p{vec1.begin(), vec1.end()};
        //new sheriff in town
        assert(p.top() == 63);
        auto ptr1 = p.addNode(11);
        assert(p.size() == 6);
        p.updateElt(ptr1, 64);
        assert(p.top() == 64);
       ...
       assert(p.top() == 51);
        ...
        std::cout << " testing Update priorities \n";
